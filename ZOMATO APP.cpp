...
class HASHING
{
		...
		while(1){
			...
			switch(choice)
		    {
		        ...
		        case 3:{
					...
					ofstream PFile;
					PFile.open("DECRYPTED_PASSWORDS.txt.txt", ios::app);
	...
	{
	    ofstream PFile;
		PFile.open("PASSWORDS.txt", ios::app);
	    PFile << admin_pwd <<endl;
	}
...
class restaurant{
	...
	city;
		...
		vector<restaurant> read_file_without_parser(){
			...
			ifstream file("zomato.csv");
			CSVReader reader("zomato.csv");
			char c;
			while(c!='\n'){
				c= file.get();
			}
